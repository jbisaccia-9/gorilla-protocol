...
if grep -RqE --include='*.cpp' --include='*.h' \
  '(/Engine/BasicShapes/|BasicShapeMaterial|BuildCoastalFacility|ConfigureViewModelPrimitive|DrawText\(|ConstructorHelpers::FObjectFinder|NewObject<UInput(Action|MappingContext)>)' Source; then
  echo "A prohibited primitive or canvas presentation fallback was reintroduced." >&2
  exit 1
fi
